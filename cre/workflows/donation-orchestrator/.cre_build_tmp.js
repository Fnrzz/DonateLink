// workflows/donation-orchestrator/.cre_build_tmp.js
...
function toBase64(str) {
  ...
  for (let i2 = 0;i2 < str.length; i2 += 3) {
    ...
    const b = i2 + 1 < str.length ? str.charCodeAt(i2 + 1) : 0;
    const c = i2 + 2 < str.length ? str.charCodeAt(i2 + 2) : 0;
    const trip = a << 16 | b << 8 | c;
    result += BASE64_CHARS[trip >> 18 & 63];
    result += BASE64_CHARS[trip >> 12 & 63];
    result += i2 + 1 < str.length ? BASE64_CHARS[trip >> 6 & 63] : "=";
    result += i2 + 2 < str.length ? BASE64_CHARS[trip & 63] : "=";
  }
  return result;
}
function decodeBody(body) {
  ...
  for (let i2 = 0;i2 < body.length; i2++) {
    str += String.fromCharCode(body[i2]);
  }
  return str;
}
var BASE_SEPOLIA_CHAIN_SELECTOR = ClientCapability.SUPPORTED_CHAIN_SELECTORS["ethereum-testnet-sepolia-base-1"];
var DONATELINK_CONTRACT_ADDRESS = "0x5217eAA2569793869caAeb8286a9Ce0854426901";
var DONATION_RECEIVED_EVENT_SIG = "0x7c0b89c70424cc72f0070e1cba7e4e29ed4e21b3a8e06b42dcd3d725b4b24906";
var handleDonation = (runtime2, triggerLog) => {
  const httpClient = new ClientCapability3;
  runtime2.log("[DonateLink CRE] Processing donation event...");
  const toHex = (bytes) => Array.from(bytes).map((b) => b.toString(16).padStart(2, "0")).join("");
  const txHash = toHex(triggerLog.txHash);
  runtime2.log(`[DonateLink CRE] TX Hash: 0x${txHash}`);
  const donor = triggerLog.topics.length > 1 ? "0x" + toHex(triggerLog.topics[1]).slice(24) : "unknown";
  const streamer = triggerLog.topics.length > 2 ? "0x" + toHex(triggerLog.topics[2]).slice(24) : "unknown";
  runtime2.log(`[DonateLink CRE] Donor: ${donor}, Streamer: ${streamer}`);
  ...
  if (data.length >= 224) {
    let usdRaw = BigInt(0);
    for (let i2 = 64;i2 < 96; i2++) {
      usdRaw = usdRaw << BigInt(8) | BigInt(data[i2]);
    }
    amountUsd = Number(usdRaw) / 1e8;
    let tokenRaw = BigInt(0);
    for (let i2 = 96;i2 < 128; i2++) {
      tokenRaw = tokenRaw << BigInt(8) | BigInt(data[i2]);
    }
    amountToken = Number(tokenRaw) / 1000000000000000000;
    tokenAddress = "0x" + toHex(data.slice(140, 160));
  }
  runtime2.log(`[DonateLink CRE] Amount USD: ${amountUsd}, Token Amount: ${amountToken}`);
  const TOKEN_SYMBOLS = {
  ...
  const recordDonation = httpClient.sendRequest(runtime2, (sendRequester) => {
    const apiUrl = runtime2.config.apiUrl || "https://donatelink.vercel.app";
  ...
  runtime2.log(`[DonateLink CRE] API Response: ${recordResult}`);
  const fetchPrices = httpClient.sendRequest(runtime2, (sendRequester) => {
    const priceResponse = sendRequester.sendRequest({
  ...
  runtime2.log(`[DonateLink CRE] Prices: ${pricesResult}`);
  const generateCelebration = httpClient.sendRequest(runtime2, (sendRequester) => {
    const geminiApiKey = runtime2.config.geminiApiKey;
    if (!geminiApiKey) {
      return "\uD83C\uDF89 Donation received! Thank you for your generosity!";
    }
    const prompt = "Generate a fun, one-line celebration message for a crypto donation. Keep it under 100 characters and make it exciting!";
    ...
    try {
      ...
      return parsed?.candidates?.[0]?.content?.parts?.[0]?.text?.trim() || "\uD83C\uDF89 Donation received!";
    } catch {
      return "\uD83C\uDF89 Donation received!";
    }
  }, consensusIdenticalAggregation());
  const celebrationMessage = generateCelebration().result();
  runtime2.log(`[DonateLink CRE] Celebration: ${celebrationMessage}`);
  runtime2.log("[DonateLink CRE] Orchestration complete");
...
var initWorkflow = () => {
  const evmClient = new ClientCapability(BASE_SEPOLIA_CHAIN_SELECTOR);
...
async function main() {
  console.log(`[DonateLink CRE] Starting workflow [${new Date().toISOString()}]`);
